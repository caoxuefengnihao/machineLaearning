...
Xtraint = pd.DataFrame(Xtraint,columns=["A","B","C","D","E","F"])
Xtestt = pd.DataFrame(Xtestt,columns=["A","B","C","D","E","F"])
fentong = pd.DataFrame(KBTT.fit_transform(Xtraint.iloc[:,0:1]))
print(Xtraint.drop(["A"],axis=1))
Xtraint = Xtraint.drop(["A"],axis=1)
Xtraint  = pd.concat([Xtraint,fentong],axis=1)

fentong1 = pd.DataFrame(KBTT.fit_transform(Xtestt.iloc[:,0:1]))
print(Xtestt.drop(["A"],axis=1))
Xtestt = Xtestt.drop(["A"],axis=1)
